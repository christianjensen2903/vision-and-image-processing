...
def ransac_3dvector(
    ...
    while k > trial_count:
        ...
        # print("Quartiles of fit:", np.percentile(fit, [75, 90, 95, 99]))
    ...
    if best_m is None:
        ...
        if verbose >= 2:
            print(
                "ransac_3dvector(): returning after {0} iterations.".format(trial_count)
            )
...
def smooth_normal_field(
    n1, n2, n3, mask, bc_list=None, iters=100, tau=0.05, verbose=False
):
    """
...
def tichonov_regularisation_normal_field(
    n1, n2, n3, mu, mask, bc_list=None, iters=10, tau=0.05, verbose=False, eps=1e-7
):
    """
...
def unbiased_integrate(n1, n2, n3, mask, order=2):
    ...
    Omega[:, :, 0] = (
        Omega_padded[2:, 1:-1] * mask
    )  # value 1 iff bottom neighbor also in mask
    Omega[:, :, 1] = (
        Omega_padded[:-2, 1:-1] * mask
    )  # value 1 iff top neighbor also in mask
    Omega[:, :, 2] = (
        Omega_padded[1:-1, 2:] * mask
    )  # value 1 iff right neighbor also in mask
    Omega[:, :, 3] = (
        Omega_padded[1:-1, :-2] * mask
    )  # value 1 iff left neighbor also in mask
    del Omega_padded
...
def display_surface_mayavi(z, albedo=None):
    ...
    mlab.view(
        azimuth=-60, elevation=25.0, focalpoint=(x2, y2, -1.0), distance=2.5 * max(m, n)
    )
    mlab.show()
